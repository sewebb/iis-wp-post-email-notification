...
function register_iis_notify_scripts() {
	if ( is_page_template( 'userfacing-template.php' ) ) {
		wp_register_script( 'iis_notify_frontend', plugins_url( 'js/bundle/frontend-widget.js', __FILE__ ), array(), '20170822', true );
		global $wpdb;
		...
		if ( strpos( $test_home, 'webbstjarnan.nu' ) ) {
			$adminurl = $test_home . '/wp-admin/admin-ajax.php';
		} else {
